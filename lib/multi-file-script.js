 ...
 *   Source.
 * 
 * This file returns the same script sources as single-file-script.js but
 * changes the initialization function to initialize the multifile class instead.
 */
...
function initMultiFile(constants) {
	...
	// This calls into init() from single-file-core/single-file.js
	window.singlefile.initMulti({
		fetch: async (url, options) => {
			console.debug("fetch AAA", url, options);// FIXME why is this not getting called?
			try {
...
async function getScriptSource(options) {
	...
	source += "(" + initMultiFile.toString() + ")(" + JSON.stringify({
		FETCH_FUNCTION_NAME,
		RESOLVE_FETCH_FUNCTION_NAME,
		REJECT_FETCH_FUNCTION_NAME
	}) + ");";
...
function getPageDataScriptSource(options, [SET_SCREENSHOT_FUNCTION_NAME, SET_PDF_FUNCTION_NAME, SET_PAGE_DATA_FUNCTION_NAME, CAPTURE_SCREENSHOT_FUNCTION_NAME, PRINT_TO_PDF_FUNCTION_NAME]) {
	...
	return window.singlefile.getPageDataAndResources1(options).then(data => {
